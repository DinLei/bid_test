...
def get_log_price(date, mode, units):
    ...
    for unit in units_arr:
        print("In getPrice: date={}, mode={}, unit={}".format(date, mode, unit))
        cmd = log_price % (date, mode, unit)
        file = "./logs/{}_{}_{}_price.log".format(date, mode, unit)
        text = exec_cmd(cmd)
...
def get_log_imp(date, units):
    units_arr = units.split(",")
    for unit in units_arr:
        print("In getImp: date={}, unit={}".format(date, unit))
        cmd = log_imp % (date, unit)
        file = "./logs/{}_{}_imp.log".format(date, unit)
        text = exec_cmd(cmd)
        write_file(file, text)
